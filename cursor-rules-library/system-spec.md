...
4. For permissions / routes / UUID strategy: read `PLATFORM_HARDENING_PLAN.md` when working on those areas.

If behavior is unclear, **update the spec first**, then implement.

## Roles (4 account types)

- `ADMIN` → UI: **compte admin**
- `OFFICE_USER` → UI: **ملحق بالديوان** / Attaché de cabinet (not raw enum)
- `CHEF_CABINET` → UI: **رئيس الديوان** / Chef de cabinet
- `WALI` → UI: **compte wali**

Never expose raw enum names in UI. Communes / dairas / directions are **reference data only** (no login accounts for those yet).
...
- Shared (when present): `shared/access-policy`, `shared/routes`.
