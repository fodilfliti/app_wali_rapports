...
- **Hide rapport / type** (office): soft-hide individual rapports (`rapports.hidden_at`) or rapport types (`rapport_types.hidden_at`) from service hubs; **fiche_lecture** type cannot be hidden as a type. Restore from « hidden » filter. Confirm dialog before hide.
  ...
  - **Wali / Chef hubs:** soft-hidden types are **never** shown (no office action-badge exception). Soft-hidden rapports are excluded from lists and cannot be opened by direct URL (`404`). Opening a `table_grid` or `commune_list` type with **exactly one** visible rapport for that office user **auto-opens** `/rapports/:id/view` (replace). Scoped type lists must filter by `service_id`, `rapport_type_id`, and `office_user_id` (Chef must not drop those filters on the inbox endpoint).
- **Hard-delete unused rapport type** (office `manage`): permanent delete when the type has **zero** `rapports` rows (any status / soft-hidden). Never for `fiche_lecture`. If in use → `409` and UI offers soft-hide instead. When deleting an unused type, also delete its orphan table schema if the schema is non-`is_system`, owned by the service, and no other type references its slug. Audit: `RAPPORT_TYPE_DELETE`. Hub list exposes `can_delete` per type.
- **Office content hub (guided create):** for office `manage`, the leaf service hub always shows sections for **ملف مركّب / جدول / قائمة** (even when empty) with a dashed **add** tile at the end of each section that opens an in-hub create modal with that `content_kind` prefilled — no kind dropdown. **مذكرة استخلاصية** has no add tile (exactly one auto-created type). جدول add opens a **path chooser** first: (1) table with new columns → **type name** first, then **own schema name** (may differ from type) + columns → schema+type; (2) table from existing schema → type name + pick schema → type only; (3) schema columns only → **schema name** + columns → schema without type (reuse later). User can go back and change path anytime. جدول section header has **مخططات الأعمدة** → search + select schema, then table preview (Data/Schema tabs); edit name, edit columns, delete unused. قائمة add asks **complex vs table** with large choice cards; table mode with new columns also asks type name first then own schema name + columns (or pick existing schema). Hub header keeps only **إعداد المجال** (advanced config) — not separate « مخطط جديد / نوع تقرير جديد » shortcuts.
- **Kinds / versions explainer:** office / Wali / Chef leaf service hub opens it via a header button (next to **إعداد المجال** for office manage; label describes contents: kinds, versions, liste modes) as a modal; **إعداد المجال** config page keeps the same block at the **end of the page**. Covers type vs rapport, the four content kinds, liste complex vs table modes, and versioned vs standalone.
- **Editors how-to explainer (office only):** second header button on the leaf service hub opens a separate modal covering the common create/save/submit flow and, per editor, create steps + capabilities: document/fiche rich editor, `table_grid` tools, liste **complex** (per-entity document; submit on hub), and liste **table** (per-entity + bulk grid; submit on hub).
...
- **Read-only version open** (office / Wali / Chef): resolve snapshot content by `content_kind`:
  ...
  - **Finished rows** (`_row_finished`): hidden in archive / Wali–Chef read-only table views (same as office “Actives” filter). They remain in the snapshot JSON for export options that include finished rows.
