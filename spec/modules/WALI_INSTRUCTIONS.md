...
Paths: `/governor`, `/cabinet`, `/chief` (`ROUTES.md`). Public ids UUID (`IDENTITY_UUID.md`). Create gated by `rapports.instructions.create` / ActionKey (`ACCESS_PROFILES.md`).
...
- Office `unread_notifications` **and** `GET /cabinet/notifications` **exclude** `waliInstruction` (and broadcast keys) so Instructions / Shared tiles are not double-counted on the Notifications tile or header bell.
- Marking a notification read (`PATCH /cabinet/notifications/:id/read`) also sets the matching instruction/broadcast recipient `read_at` when present.
...
| Method | Path | Role |
| ------ | ---- | ---- |
| `POST` | `/governor/instructions` | Wali create (multipart or JSON + file ids) |
...
| `WALI_INSTRUCTION_DELETE` | Wali deletes instruction (+ related notifs) |
