...
6. **Office delete** — Éditeur (`manage`); always a confirm dialog first (copy depends on path); then a snackbar explaining the outcome:
   ...
   - **Instant discard current version (multi-version, `versioning_mode=versioned`, no Chef/Wali action):** older `rapport_versions` exist → hard-delete **only** the current version (and its discussion/notifications linked to that version); create a **new draft version** that is a **copy of the previous version’s `data_json`** (new `version_number`, no `submitted_at`); set `status = draft`, `chef_gate = required`. Older archive versions stay. Confirm + snack: this version only deleted; continue from restored previous copy as brouillon.
   - **Instant reset to fresh v1 (`versioned` sole version, no Chef/Wali action):** only one version and no responses → wipe content/side-effects and create a **new empty draft version 1** on the same rapport row (do **not** destroy the rapport). Confirm + snack: cleared; start again from version 1. **Not** used for `standalone` types (fiche lecture / fichier complexe) — those have no real version history.
   - **Instant full delete (`standalone` or any type with no Chef/Wali action and no version rollback path):** hard-delete the entire rapport. Confirm + snack: deleted permanently (no « start from 0 »).
