...
function needsTableSchema(kind: string) {
  return kind === 'table_grid' || kind === 'commune_list'
}
...
export function OfficeServiceConfigPage({ token }: Props) {
  ...
  async function saveRapportType() {
    ...
    try {
      await api.createOfficeServiceRapportType(token, sid, {
        ...
        table_schema_slug: needsTableSchema(typeForm.content_kind) ? typeForm.table_schema_slug : undefined,
      })
      setTypeModal(false)
      load()
      snack.show(t('save'), 'success')
  ...
  async function saveEditTypeSchema() {
    ...
    try {
      await api.patchOfficeRapportType(token, editingType.id, { table_schema_slug: editTypeSchemaSlug })
  ...
  return (
    <div className="page">
      ...
      <div className="section">
        ...
        <TablePagination page={schemaPage} total={schemas.length} onPageChange={setSchemaPage} compact />
      </div>
      ...
      <div className="section">
          ...
          <table>
            ...
            <tbody>
              {pagedRapportTypes.map((rt) => {
                ...
                const missingSchema = needsTableSchema(rt.content_kind) && !slug
                return (
                  <tr key={rt.id}>
                    <td>{localizedName(rt, i18n.language)}</td>
                    <td>{t(`contentKind_${rt.content_kind}`)}</td>
                    <td className={missingSchema ? 'muted' : undefined}>
                      {needsTableSchema(rt.content_kind)
                        ? slug
                          ? allSchemas.find((s) => s.slug === slug)
                            ? localizedName(allSchemas.find((s) => s.slug === slug), i18n.language)
                            : slug
                    ...
                    <td>
                      {needsTableSchema(rt.content_kind) ? (
                        <button type="button" className="btn btn-secondary btn-sm" onClick={() => openEditTypeModal(rt)}>
                          {t('edit')}
                        </button>
                      ) : null}
                    </td>
        ...
        <TablePagination page={typePage} total={rapportTypes.length} onPageChange={setTypePage} compact />
      </div>

      <DocumentTemplatesSection token={token} serviceId={sid} rapportTypes={rapportTypes} />
      ...
      {typeModal ? (
        <div className="modalOverlay">
          <div className="modalCard">
            ...
            {needsTableSchema(typeForm.content_kind) ? (
              <label>
                {t('linkedSchema')}
                <select
      ...
      {editTypeModal && editingType ? (
        <div className="modalOverlay">
          <div className="modalCard">
            ...
            <label>
              {t('linkedSchema')}
              <select value={editTypeSchemaSlug} onChange={(e) => setEditTypeSchemaSlug(e.target.value)}>
                <option value="">{t('selectSchema')}</option>
                {allSchemas.map((s) => (
                  <option key={s.id} value={s.slug}>
                    {localizedName(s, i18n.language)}
            ...
            <div className="modalActions">
              <button type="button" className="btn btn-primary" onClick={saveEditTypeSchema}>
                {t('save')}
              </button>
              <button
                type="button"
