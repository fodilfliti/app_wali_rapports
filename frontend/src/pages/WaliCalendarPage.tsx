...
import { useCalendarWeekQuery } from '../hooks/queries/useListQueries'
...
export function WaliCalendarPage({ token, reviewer = 'wali' }: Props) {
  ...
  const calendarQuery = useCalendarWeekQuery(token, anchor, reviewer)
  const data = calendarQuery.data
  const isInitialLoading = calendarQuery.isLoading && !data
  const isRefreshing = calendarQuery.isFetching && !calendarQuery.isLoading
